-- Migration: Nettoyage des VIEWs inutiles
-- Date: 2025-09-30
-- Suppression des VIEWs temporaires créées pendant le diagnostic

begin;

-- Supprimer la VIEW de fusion créée pour le workaround
drop view if exists public.v_emission_factors_merged cascade;

-- Supprimer la VIEW emission_factors_algolia (obsolète)
drop view if exists public.emission_factors_algolia cascade;

-- Remettre rebuild_emission_factors_all_search() à sa version normale (sans la VIEW)
...
begin
  ...
  -- Admin (base commune) depuis emission_factors directement
...
-- Mettre à jour refresh_ef_all_for_source() aussi
