-- Safer casts for FE and Date (accept text/empty strings), redefine rebuild & refresh
...
begin
  ...
  raise notice 'ef_all projection rebuilt (casts safe): % rows', (select count(*) from public.emission_factors_all_search);
end;
$$;
...
begin
  ...
  raise notice 'ef_all refreshed (casts safe) for source: %', p_source;
end;
$$;
