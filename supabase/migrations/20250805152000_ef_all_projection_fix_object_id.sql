-- Fix: utiliser ef.id comme object_id (stable_id non présent en V1)
...
begin
  ...
  raise notice 'ef_all projection rebuilt (fix object_id=id): % rows', (select count(*) from public.emission_factors_all_search);
end;
$$;
...
begin
  ...
  raise notice 'ef_all refreshed (fix object_id=id) for source: %', p_source;
end;
$$;
