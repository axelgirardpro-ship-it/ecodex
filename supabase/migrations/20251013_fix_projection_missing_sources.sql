-- Migration: Fix projection pour inclure TOUTES les sources même si absentes de fe_sources
-- Date: 2025-10-13
-- Bug: rebuild_emission_factors_all_search() utilise INNER JOIN sur fe_sources
--       ce qui exclut 145,830 lignes (Carbon Minds, Ecoinvent, etc.)
-- Solution: Utiliser LEFT JOIN avec valeur par défaut pour access_level
...
BEGIN
  ...
  -- ✅ FIX: LEFT JOIN au lieu de JOIN pour inclure toutes les sources
  ...
  SELECT
    ...
    COALESCE(fs.access_level, 'free') AS access_level,  -- ✅ FIX: COALESCE pour valeur par défaut ('free' ou 'paid')
    (
  ...
  LEFT JOIN public.fe_sources fs ON fs.source_name = ef."Source";  -- ✅ FIX: LEFT JOIN
...
-- Même fix pour refresh_ef_all_for_source()
...
BEGIN
  ...
  SELECT
    ...
    'public' AS scope,
    NULL AS workspace_id,
    COALESCE(fs.access_level, 'free') AS access_level,  -- ✅ FIX: COALESCE ('free' ou 'paid')
    (
      SELECT array_agg(ws.workspace_id)
      FROM public.fe_source_workspace_assignments ws
      WHERE ws.source_name = ef."Source"
...
COMMENT ON FUNCTION public.rebuild_emission_factors_all_search() IS 'Rebuild complet de emission_factors_all_search - utilise LEFT JOIN pour inclure toutes les sources';
COMMENT ON FUNCTION public.refresh_ef_all_for_source(text) IS 'Rafraîchit emission_factors_all_search pour une source - utilise LEFT JOIN pour inclure toutes les sources';
