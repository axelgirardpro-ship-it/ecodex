-- First, let's fix the handle_new_user function to create workspaces
...
-- Create workspaces for existing users who don't have them
INSERT INTO public.workspaces (id, name, owner_id, plan_type)
SELECT 
  c.id,
  c.name,
...
LEFT JOIN public.workspaces w ON w.id = c.id
WHERE w.id IS NULL;