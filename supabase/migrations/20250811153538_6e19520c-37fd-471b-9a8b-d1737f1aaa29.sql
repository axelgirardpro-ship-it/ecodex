-- Fonction pour normaliser les emails (enlever la partie +)
...
-- Ajouter une colonne pour l'email normalisé
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS normalized_email TEXT;

-- Créer un index unique sur l'email normalisé pour les workspaces freemium
CREATE UNIQUE INDEX IF NOT EXISTS idx_users_normalized_email_freemium 
ON public.users (normalized_email) 
WHERE plan_type = 'freemium';

-- Mettre à jour la fonction handle_new_user pour inclure la vérification
...
-- Mettre à jour les emails normalisés existants
UPDATE public.users 
