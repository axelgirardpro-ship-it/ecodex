-- Migration: Nettoyage automatique des sources obsolètes
-- Date: 2025-10-30
-- Objectif: 
--   1. Supprimer immédiatement les sources is_global=true sans enregistrements
--   2. Modifier run_import_from_staging() pour nettoyer automatiquement après chaque import
--   3. Modifier fe_sources_with_counts pour utiliser INNER JOIN (vue propre)

-- =====================================================================
-- PARTIE 1: Nettoyage immédiat des 4 sources obsolètes
-- =====================================================================

-- Supprimer les sources is_global=true qui n'ont aucun enregistrement dans emission_factors_all_search
...
-- =====================================================================
-- PARTIE 2: Modifier la vue fe_sources_with_counts (INNER JOIN)
-- =====================================================================

-- Remplacer LEFT JOIN par INNER JOIN pour ne montrer que les sources avec des données
...
INNER JOIN public.emission_factors_all_search efs  -- INNER au lieu de LEFT
...
INNER JOIN garantit que seules les sources ayant des données réelles sont retournées.
Utilisée par le frontend /admin pour afficher uniquement les sources actives.
Les sources obsolètes sont automatiquement nettoyées lors des imports admin.';

-- =====================================================================
-- PARTIE 3: Modifier run_import_from_staging() pour nettoyage automatique
-- =====================================================================
...
DECLARE
  ...
  v_cleaned_sources int := 0;  -- Nouveau: compteur de sources nettoyées
BEGIN
  ...
  -- =====================================================================
  -- NOUVEAU: Nettoyer les sources is_global=true sans enregistrements
  -- =====================================================================
  BEGIN
    DELETE FROM public.fe_sources
    WHERE is_global = true
    ...
    GET DIAGNOSTICS v_cleaned_sources = ROW_COUNT;
    
    IF v_cleaned_sources > 0 THEN
      RAISE NOTICE 'Nettoyé % source(s) obsolète(s) sans enregistrements', v_cleaned_sources;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    PERFORM public.log_import_error('Cleanup sources', SQLERRM, NULL);
    v_cleaned_sources := -1;
  END;
  ...
  RETURN json_build_object(
    ...
    'cleaned_sources', v_cleaned_sources,
...
'Import admin principal avec nettoyage automatique des sources obsolètes.
Après le rebuild de emission_factors_all_search, supprime automatiquement
les sources is_global=true qui n''ont aucun enregistrement.
Retourne cleaned_sources pour traçabilité.';
