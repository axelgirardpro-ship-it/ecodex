-- Migration finale pour implémenter le plan de simplification des workspaces
-- Approche: convertir les rôles admin existants en supra_admin dans Global Administration
...
BEGIN
    ...
    LOOP
        ...
        -- Vérifier s'il existe déjà un rôle admin pour cet utilisateur dans Global Administration
        ...
        IF existing_admin_count > 0 THEN
            -- Mettre à jour le rôle admin existant vers supra_admin
            RAISE NOTICE 'Converting existing admin role to supra_admin for user %', supra_admin_record.user_id;
            UPDATE user_roles 
            SET role = 'supra_admin',
                updated_at = now()
            WHERE user_id = supra_admin_record.user_id 
            AND workspace_id = global_admin_workspace_id 
            AND role = 'admin';
        ELSE
            -- Mettre à jour le rôle supra_admin de NULL vers Global Administration
        ...
        -- Dans tous les cas, supprimer l'ancien rôle avec workspace_id = NULL s'il existe encore
