...
var _initProto;
class VercelLLM extends llms.ToolCallLLM {
    static{
    ...
    constructor({ model }){
        super(), this.supportToolCall = (_initProto(this), true);
        this.model = model;
    }
    get metadata() {
        return {
    ...
    toVercelMessages(messages) {
        return messages.map((message)=>{
            ...
            if (message.role === "user") {
                return {
                    role: message.role,
                    content: typeof message.content === "string" ? message.content : message.content.map((contentDetail)=>{
                        ...
                        throw new Error(`Unsupported content type: ${contentDetail.type}`);
                    })
                };
            }
        ...
        return {
            ...
            message: {
                ...
                options: result.toolCalls?.length ? {
                    toolCall: result.toolCalls.map(({ toolCallId, toolName, input })=>({
                            id: toolCallId,
                            name: toolName,
                            input
                        }))
 ...
 */ const vercel = (init)=>new VercelLLM(init);

function llamaindex({ model, index, description, options }) {
    const llm = new VercelLLM({
        model
    });
    return global.Settings.withLLM(llm, ()=>{
        const queryEngine = index.asQueryEngine();
        return ai.tool({
            description: description ?? "Get information about your documents.",
            inputSchema: zod.z.object({
                query: zod.z.string().describe("The query to get information about your documents.")
            }),
            execute: async ({ query })=>{
                ...
                if (options?.fields) {
                    const resultWithFields = {
                        ...result,
                        ...Object.fromEntries(options.fields.map((field)=>[
                                field,
                                result[field]
                            ]))
                    };
...
exports.VercelLLM = VercelLLM;
exports.llamaindex = llamaindex;
